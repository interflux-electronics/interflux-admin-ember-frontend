...
export default class ReusablesSearchComponent extends Component {
  ...
  get showDropdown() {
    return (
      this.focus && this.query && this.query !== this.labelFor(this.selected)
    );
  }
  ...
  @action
  highlight(i) {
    this.set('highlight', i);
  }

  @action
  select(option) {
    this.onSelect(option);
    // TODO: find Ember way of deselecting
  ...
  labelFor(option) {
    ...
    const keys = this.optionLabel.split('.');
    return keys.reduce((acc, value) => {
      return acc[value];
    }, option);
  }

  // TODO: Look for Ember alternative
  didInsertElement() {
    document.addEventListener('mousedown', this.onBodyClick, false);
  }

  // TODO: Look for Ember alternative
  willDestroy() {
    document.removeEventListener('mousedown', this.onBodyClick, false);
  }
  ...
  onBodyClick(event) {
    ...
    this.setProperties({ focus: clickedInside });
  }
  ...
  onBlur() {
    console.debug('<Search> onBlur()');
    this.onBlur();
  }

  @action
  onFocus() {
    console.debug('<Demo> onFocus()');
  }

  // TODO: Is this needed in Ember?
